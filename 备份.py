...
async def parse_question(question, session):
    ...
    if not attribute:
        response = await client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": "You are an assistant who helps to parse car data queries. Please map user questions to attributes (such as range, battery capacity, price, etc.)"},
                {"role": "user", "content": f"Question：What is the {question} attribute for the car data?"}
            ],
            max_tokens=50,
            temperature=0.5,
        )
...
async def answer_question(question, session):
    """
      The user asks directly. If there is only one matching model, the query is made directly. Otherwise, the user is asked to choose.
      Here, the original query results are also printed first, and then only the LLM rewrite is returned in the end.
    """
    try:
        # First get the new model and relationship from parse_question
        parsed_entity, relationship = await parse_question(question, session)

        # If parse_question gets a new entity, use it directly
        if parsed_entity:
            entity = parsed_entity
        # parse_question doesn't recognize the new model, but if the user writes "this car", etc.
        # use session.last_entity
        else:
        ...
        # If there is a car model in the session, it will be used first.
        # if session.get_last_entity():
        #     print(f"Use stored models: {session.get_last_entity()}")
        #     entity = session.get_last_entity()

        # Save the final entity to session.last_entity
        ...
        if len(matching_models) > 1:
            # Multiple models, give users a list
        ...
        if not results:
            ...
            # First print the Neo4j original query results
            print("\n--- Neo4j raw query results (Raw Result) ---")
            print(extracted_context)
            print("--- End of Raw Result ---\n")
            ...
            # Let LLM rewrite with history
            #llm_answer = await generate_response_with_openai(session.history, extracted_context)

            # LLM rewrite without history
            llm_answer = await generate_response_with_openai_no_history(session.history, extracted_context)


            bot_message = llm_answer  # Finally, only the content rewritten by LLM is output
