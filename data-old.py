...
# 定义主页面 URL
main_url = "https://ev-database.org/#sort:path~type~order=.rank~number~desc|rs-price:prev~next=10000~100000|rs-range:prev~next=0~1000|rs-fastcharge:prev~next=0~1500|rs-acceleration:prev~next=2~23|rs-topspeed:prev~next=110~350|rs-battery:prev~next=10~200|rs-towweight:prev~next=0~2500|rs-eff:prev~next=100~350|rs-safety:prev~next=-1~5|paging:currentPage=0|paging:number=10"
driver.get(main_url)
time.sleep(3)
...
def extract_data(car_url):
    ...
    print(f"Extracted data for {data['Model']} from {car_url}")
    return data
...
for i, car_url in enumerate(car_links):
    ...
    car_data = extract_data(car_url)
    all_car_data.append(car_data)

# 保存所有数据到 CSV 文件
df = pd.DataFrame(all_car_data)
df.to_csv("All_Cars_Data.csv", index=False, encoding='utf-8-sig')
print("All car data has been saved to All_Cars_Data.csv")
